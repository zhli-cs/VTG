        ...
        if use_generator:
            ...
            for idx in self.poison_samples_idx:
                ...
                if perturb_type == 'samplewise':
                    ...
                    noise = noise.squeeze(0).mul(255).clamp_(0, 255).permute(1, 2, 0).detach().cpu().numpy() 
        ...
        if use_generator:
            ...
            for idx in self.poison_samples_idx:
                ...
                if perturb_type == 'samplewise':
                    # Sample Wise Poison
                    noise = self.generator(data_input)  
                    noise = noise.squeeze(0).mul(255).clamp_(0, 255).detach().cpu().numpy()  
                    if add_uniform_noise: # default: False
                        noise += np.random.uniform(0, 8, (32, 32, 3))
                    # 添加噪声
                    self.data[idx] = self.data[idx] + noise
